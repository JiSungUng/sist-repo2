      ...
      <table class="table table-bordered" style="width: 600px;">
            ...
            <td>
               <h3>
                  <b>${dto.subject }</b> <span
                     style="color: gray; float: right; font-size: 13px;"><fmt:formatDate
                        value="${dto.writeday }" pattern="yyyy-MM-dd HH:mm" /></span>
               </h3> <span>작성자: ${dto.name }(${dto.myid })</span> <c:if
                  test="${dto.uploadfile!='no' }">
                  <span style="float: right;"> <a
                     href="download?clip=${dto.uploadfile}"> <!-- 다운로드, downloadcontroller매핑 -->
         ...
         <tr>
            <td><c:if test="${bupload==true }">
                  <h3>업로드 파일이 이미지</h3>
                  <img alt="" src="../photo/${dto.uploadfile }"
                     style="max-width: 200px;">
               </c:if> <br>
            <br> <pre style="background-color: #fff; border: 0px;">${dto.content }</pre>
               ...
               <c:if test="${sessionScope.loginok!=null }">
                  <div class="aform">
                     ...
                     <input type="hidden" id="num" value="${dto.num }">
                        ...
                        <button type="button" class="btn btn-detfault" id="btnansweradd">등록</button>
                     </div>
                  </div>
               </c:if>
            </td>
         ...
         <tr>
            <td align="right">
               <!-- 글쓰기는 로그인중일때만 --> <c:if test="${sessionScope.loginok!=null }">
                  <button type="button" class="btn btn-default"
                     onclick="location.href='form'">글쓰기</button>
               </c:if>
               <button type="button" class="btn btn-default"
                  onclick="location.href='list?currentPage=${currentPage}'">목록</button> <!-- 로그인중이면서 자기글만 수정, 삭제 버튼 보이게 -->
